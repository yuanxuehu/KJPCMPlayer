...
OSStatus audioConverterComplexInputDataProc(AudioConverterRef inAudioConverter,UInt32 *ioNumberDataPacket,AudioBufferList *ioData,AudioStreamPacketDescription **outDataPacketDescription,void *inUserData) {
    ...
    KJFillComplexInputParm *param = (KJFillComplexInputParm *)inUserData;
...
@interface KJAACEncoder () {
    KJConverterContext *convertContext;
...
- (instancetype)init {
    if ( self = [super init]) {
        encodeQueue = dispatch_queue_create("aacEncode", DISPATCH_QUEUE_SERIAL);
    }
    return self;
}
...
- (void)setUpConverter:(CMSampleBufferRef)sampleBuffer {
    ...
    KJConverterContext *context = malloc(sizeof(KJConverterContext));
...
- (void)encodeSmapleBuffer:(CMSampleBufferRef)sampleBuffer {
    ...
    KJConverterContext *cxt = self->convertContext;
    if (cxt && cxt->converter) {
        ...
        dispatch_async(encodeQueue, ^{
            ...
            if (status != noErr) {
                ...
                KJFillComplexInputParm userParam;
